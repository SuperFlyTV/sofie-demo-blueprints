import { IBlueprintAdLibPiece, IShowStyleUserContext, PieceLifespan, TSR } from "@sofie-automation/blueprints-integration";
...
export function getGlobalAdlibs (context: IShowStyleUserContext): IBlueprintAdLibPiece[] {
    ...
    return [
        ...config.atemSources.filter(source => source.type === AtemSourceType.Camera).map((source, i) => makeCameraAdlib(i, source.input)),
        ...config.atemSources.filter(source => source.type === AtemSourceType.Remote).map((source, i) => makeRemoteAdlib(i, source.input)),
    ]
}